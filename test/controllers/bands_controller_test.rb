...
class BandsControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should create band" do
    assert_difference('Band.count') do
      post bands_url, params: { band: { bio: @band.bio, genre: @band.genre, name: @band.name, status: @band.status } }
    end
  ...
  test "should update band" do
    patch band_url(@band), params: { band: { bio: @band.bio, genre: @band.genre, name: @band.name, status: @band.status } }
